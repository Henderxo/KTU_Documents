    ...
    public class ItemRepo
    {
        ...
        public static void DeleteItem(int itemId)
        {
            ...
            foreach(Post s in Posts)
            {
                if(s.ItemId == itemId)
                {
                    CommentRepo.DeleteCommentsByPostID(s.Id);
                    LikesRepo.DeletelIKESsByPostID(s.Id);
                    // Delete the post
                    PostRepo.DeletePost(s.Id);
                }
                
            }

            // Now, write your SQL query for deleting the item from the items table
            var itemQuery = "DELETE FROM items WHERE item_ID = ?itemId";

            // Execute the query for deleting the item from the items table
            Sql.Delete(itemQuery, args =>
            {
                args.Add("?itemId", itemId);
            });
        }
