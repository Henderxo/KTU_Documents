...
public class HashMap<K, V> implements EvaluableMap<K, V> {
    ...
    public V remove(K key) {
        ...
        System.out.println("MaxCahin Size: " + maxChainSize);
        System.out.println("Max rehashes counter: " + rehashesCounter);
        System.out.println("Last updated chain: " +lastUpdatedChain);
        System.out.println("Chains counter: " + chainsCounter);*/
        V removed = null;
        if(key == null)
        {
        ...
        Node<K, V> s = table[index];

        for (Node<K, V> n = node; n != null; n = n.next) {

            if((n.key).equals(key) && n == s)
            {
                table[index] = n.next;
                removed = s.value;
                s = null;
                ...
                lastUpdatedChain = index;
                size--;
                //UpdateMaxChainSize(table[index]);
                break;
            }
    ...
    private void UpdateMaxChainSize(Node<K,V> N)
    {
        int count = 0;
        int absoluteInt = 0;
        for (Node<K, V> node : table) {
            ...
            if(absoluteInt < count)
            {
                absoluteInt = count;
            }
            count = 0;
        }
        if(maxChainSize != absoluteInt)
        {
            maxChainSize = absoluteInt;
        }
    }
    ...
    public boolean replace(K key, V oldValue, V newValue) {
        ...
        {
            lastUpdatedChain = HashManager.hash(key.hashCode(), table.length, ht);
            put(key, newValue);
            System.out.println("After remove:");
            System.out.println("MaxCahin Size: " + maxChainSize);
            System.out.println("Max rehashes counter: " + rehashesCounter);
