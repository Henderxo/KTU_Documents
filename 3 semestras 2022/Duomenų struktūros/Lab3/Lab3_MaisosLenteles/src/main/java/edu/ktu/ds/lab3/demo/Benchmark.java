...
public class Benchmark {
    ...
    public static class FullMap {
        ...
        HashMapOa<String, Car> oaCarsMap;
        ...
        public void generateIdsAndCars(BenchmarkParams params) {
            ...
            RandCar = Benchmark.generateCars(1).get(0);
            RandId = Benchmark.generateIds(1).get(0);
        }

        @Setup(Level.Invocation)
        public void fillCarMap(BenchmarkParams params) {
            carsMap = new HashMap<>(HashManager.HashType.DIVISION);
            oaCarsMap = new HashMapOa<>(HashManager.HashType.DIVISION);
            putMappings(ids, cars, carsMap);
            putMappings(ids, cars, oaCarsMap);
        }
    }

    @Param({"10000", "20000", "30000",  "40000", "50000"})
    ...
    public void putHashMap(FullMap fullmap){
        fullmap.carsMap.put(fullmap.RandId, fullmap.RandCar);
    }

    @org.openjdk.jmh.annotations.Benchmark
    public void putOaHashMap(FullMap fullmap){
        fullmap.oaCarsMap.put(fullmap.RandId, fullmap.RandCar);
    }
