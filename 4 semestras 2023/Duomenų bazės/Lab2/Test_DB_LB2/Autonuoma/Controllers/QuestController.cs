...
public class QuestsController : Controller
{
	...
	{
		var ClassEvm = QuestsRepo.Find(id);
		return View(ClassEvm);
	}

    [HttpPost]
	public ActionResult Create(Quests klientas)
	{
		//do not allow creation of entity with 'asmensKodas' field matching existing one
		var match = QuestsRepo.Find(klientas.id);
	...
	{
		return View(QuestsRepo.Find(id));
	}

    [HttpPost]
	public ActionResult Edit(int Asmens_kodas, Quests klientas)
	{
		//form field validation passed?
		if (ModelState.IsValid)
		{
			QuestsRepo.Update(klientas);
